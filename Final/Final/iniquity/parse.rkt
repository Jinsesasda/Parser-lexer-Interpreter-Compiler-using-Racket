...
(define (parse-e s)
  (match s
    ...
    [(cons 'apply (cons (? symbol? f) es))
     (parse-apply f es)]
    [(cons 'values es) (Val (map parse-e es))]


    [(list 'let-values (list (cons vars e)) e0)  (let-val  (map parse-e vars) (map parse-e e) (parse-e e0))] 
                                                     
    [(cons (? symbol? f) es) (App f (map parse-e es))]
    [(list 'values) (Val '())]
