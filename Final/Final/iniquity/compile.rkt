...
(define rdi 'rdi) ; arg
(define rcx 'rcx)
(define r11  'r11) ;; arity number
(define r12 'r12)
(define r2 'r2)
(define rdx 'rdx)
(define type-vector #b1001)
...
(define (compile-e e c)
  ...
  (match e
    ...
    [(Val '()) (seq (Jmp 'done))]
    [(Val es)  (if (= (length es) 0)
                   '()
                (compile-values es c))]
    [(let-val vars e1 e2)  (compile-let-values vars e1 e2 c)]
...
(define (two es)
  ...
  (cond
    [(zero? (length es)) '()]
    [(= 1 (length es)) (compile-e (car es) c)]
    [else (seq (Jmp 'raise_error_align))]))
...
;; check number of 
(define (compile-let-values vars e1 e2 c)

 (cond 
  [(= 0 (length vars)) (compile-e e2 c)]

[else
   (let ((done (gensym 'done))
         (complete (gensym 'complete)))
        (seq
             (Mov r11 (length vars))
             (Mov r12 (length e1))
             (Cmp r11 r12)
...
(define (compile-begin e1 e2 c)
  (match e1
    [(Val es) (seq (Mov rax (value->bits 1))
                   (Jmp 'e) 
                    )]
