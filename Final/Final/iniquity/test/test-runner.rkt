...
(define (check-equal?* prog expected-list)
  (check-equal?
   (call-with-values
    (lambda () (apply run prog))
    (lambda xs xs))
   expected-list))


  (check-equal? (run '(begin (values 1 2) 1)) 1)
  
  



 ; (check-equal? (run '(let-values (((x y) (quote()))) 0)) 'err)
 ; (check-equal? (run '(let-values (((x y) 1)) 0)) 'err)
 ; (check-equal? (run'(values 1)) 9)
 ...
 ; (check-equal? (run '(let-values ([(x) 1]) (x))) 1))
