...
(define (explode s)
  ...
  (cond
    [(= (string-length s) 0) '()]
    [else (cons (string (string-ref s 0)) (explode (substring s 1)))]))
  ...
  (define (adjacent-pairs lst)
    (cond
      ...
      [else (cons (list (first lst) (second lst))
                  (adjacent-pairs (rest lst)))]))

  (if (< (string-length s) 2)
      '()
      (adjacent-pairs (explode s))))
...
(define (zip-add ls1 ls2)
  ...
  (cond
    [(empty? ls1) '()]                               
    [else (cons (+ (first ls1) (first ls2))         
                (zip-add (rest ls1) (rest ls2)))]))  
...
(cond
    [(empty? ls1) '()]                                 
    [else (cons (list (first ls1) (first ls2))        
                (zip-lon (rest ls1) (rest ls2)))]))    
...
(define (max-lon xs)
  ...
  (cond
    ...
    [else (let ([rest-max (max-lon (rest xs))])
...
(define (minimize f xs)
  (define (minimize-helper current-min current-elem lst)
    (cond
      [(empty? lst) current-min]                                     
      [(< (f (car lst)) (f current-min))                           
       (minimize-helper (car lst) (car lst) (cdr lst))]               
      [else (minimize-helper current-min (car lst) (cdr lst))]))      

  (minimize-helper (car xs) (car xs) (cdr xs)))
...
(define (sort < xs)
    ...
    (cond
      ((null? lst) (values '() '()))
      ((< (car lst) pivot)
       (let-values ([(small big) (partition pivot (cdr lst))])
         (values (cons (car lst) small) big)))
      (else
       (let-values ([(small big) (partition pivot (cdr lst))])
         (values small (cons (car lst) big))))))

  (cond
    ((null? xs) xs)
    (else
     (let-values ([(small big) (partition (car xs) (cdr xs))])
       (append (sort < small) (list (car xs)) (sort < big))))))
 ...
 (define (compose f g)
  ...
  (define (helper funcs)
    (cond
      ...
      (else (compose (car funcs) (helper (cdr funcs))))))
...
(define (btn-height bt)
  (match bt
    [(leaf) 0]  ; Base case: A leaf has height 0
    [(node _ left right)
     (max (+ 1 (btn-height left))   
          (+ 1 (btn-height right))) 
     ]))
...
(define (btn-gen-full h n)
   (if (= h 0)
      (leaf)  ; Base case: If height is 0, return a leaf node
...
(define (btn-contains? bt n)
  (match bt
    [(leaf) #f]  ; Base case: an empty tree does not contain any number
...
(define (btn-preorder btn)
  (match btn
    [(leaf) '()]  ; Base case: an empty tree has no numbers
    [(node value left right)
     (append (list value)            
             (btn-preorder left)      
             (btn-preorder right))   
...
(define (expr-free-vars e)
   (define (expr-free-vars-helper e bound-vars)
    (match e
      [(Int _) '()]                     
      [(Bool _) '()]                    
      [(Var v) (if (set-member? bound-vars v) '() (list v))] 
      [(App e1 e2) (append (expr-free-vars-helper e1 bound-vars) (expr-free-vars-helper e2 bound-vars))]  
      [(Lam x e) (expr-free-vars-helper e (set-add bound-vars x))]))  
    (expr-free-vars-helper e (set)) 
)
(module+ test
